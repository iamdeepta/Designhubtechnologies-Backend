...
const HomeSection7MainEdit = ({
  ...
  const [main_description_up_faq, setMainDescriptionUpFaq] =
    useState(description_data_faq);

  //update data
  async function updateMainDataFaq(id) {
    const formData = new FormData();
    formData.append("main_title_up_faq", main_title_up_faq);
    formData.append("main_description_up_faq", main_description_up_faq);

    let result_faq = await fetch(
      AppUrl.base_url + "homesection7MainUpdate/" + id,
      {
        method: "POST",
        body: formData,
      }
      ...
      <div className="card-body">
        ...
        <div className="form-group">
          <input
            ...
            className="form-control"
            value={main_description_up_faq}
            onChange={(e) => setMainDescriptionUpFaq(e.target.value)}
          />
        </div>
...
export default HomeSection7MainEdit;
